...
(Azure DevOps in this specification version), creates an isolated workspace for each issue, and runs a
...
- Issue tracker API (Azure DevOps for `tracker.kind: ado` in this specification version).
...
- `kind` (string)
  ...
  - Current supported value: `ado`
- `endpoint` (string)
  - Required when `tracker.kind == "ado"`; no built-in default.
  - Provide the Azure DevOps organization root URL (example: `https://dev.azure.com/{organization}`).
- `api_key` (string)
  - May be a literal token or `$VAR_NAME`.
  - For `tracker.kind == "ado"`, this is an Azure DevOps Personal Access Token (PAT) with at least Work Items read permission.
  - Canonical environment variable for `tracker.kind == "ado"`: `ADO_PAT`.
  - If `$VAR_NAME` resolves to an empty string, treat the key as missing.
- `project_slug` (string)
  - The Azure DevOps project name.
  - Required for dispatch when `tracker.kind == "ado"`.
- `active_states` (list of strings or comma-separated string)
  - Default: `New`, `Active`
- `terminal_states` (list of strings or comma-separated string)
  - Default: `Closed`, `Resolved`, `Done`, `Cancelled`
...
- If the workflow prompt body is empty, the runtime may use a minimal default prompt
  (`You are working on a work item from Azure DevOps.`).
...
- `tracker.kind`: string, required, currently `ado`
- `tracker.endpoint`: string, required when `tracker.kind=ado`; Azure DevOps organization URL (example: `https://dev.azure.com/{organization}`)
- `tracker.api_key`: string or `$VAR`, canonical env `ADO_PAT` when `tracker.kind=ado`; must be an Azure DevOps PAT with Work Items read permission
- `tracker.project_slug`: string, required when `tracker.kind=ado`; the Azure DevOps project name
- `tracker.active_states`: list/string, default `New, Active`
- `tracker.terminal_states`: list/string, default `Closed, Resolved, Done, Cancelled`
...
This is not the same as tracker states (`New`, `Active`, etc.). This is the service's internal
claim state.
...
- Blocker rule for `New` state passes:
  - If the issue state is `New`, do not dispatch when any blocker is non-terminal.
...
- Current optional standardized tool: `ado_api`.
- If implemented, supported tools should be advertised to the app-server session during startup
  using the protocol mechanism supported by the targeted Codex app-server version.
- Unsupported tool names should still return a failure result and continue the session.

`ado_api` extension contract:

- Purpose: execute a REST API call against Azure DevOps using Symphony's configured tracker auth
  for the current session.
- Availability: only meaningful when `tracker.kind == "ado"` and valid ADO auth is configured.
- Preferred input shape:
  ...
  For WIQL queries, the preferred input shape is:

  ```json
  {
    "wiql": "SELECT [System.Id], [System.Title], [System.State] FROM WorkItems WHERE [System.TeamProject] = @project AND [System.State] = 'Active'"
  }
  ```

- `method` must be one of `GET`, `POST`, `PATCH`, `DELETE`.
- `path` must be a non-empty string scoped to the configured ADO organization.
- `body` is optional and, when present, must be a JSON-serializable object.
- `wiql` may be provided instead of `method`/`path` as a shorthand for executing a WIQL query.
- Execute one REST operation per tool call.
- Reuse the configured ADO organization URL and PAT auth from the active Symphony workflow/runtime
  config; do not require the coding agent to read raw tokens from disk.
- Tool result semantics:
  - HTTP 2xx response -> `success=true` with response body
  - HTTP 4xx/5xx response -> `success=false` with status code and response body for debugging
  - invalid input, missing auth, or transport failure -> `success=false` with an error payload
- Return the ADO response or error payload as structured tool output that the model can inspect
  in-session.
...
## 11. Issue Tracker Integration Contract (ADO-Compatible)
...
### 11.2 Query Semantics (Azure DevOps)

Azure DevOps-specific requirements for `tracker.kind == "ado"`:

- `tracker.kind == "ado"`
- REST API base URL: `{tracker.endpoint}/{tracker.project_slug}/_apis/wit` (API version `7.1`)
- Auth: HTTP Basic authentication with base64-encoded `:ADO_PAT` (empty username, PAT as password)
- `tracker.project_slug` maps to the Azure DevOps project name
- Candidate issue fetch uses WIQL (`POST /_apis/wit/wiql`), filtering by project and active states:
  `SELECT [System.Id] FROM WorkItems WHERE [System.TeamProject] = @project AND [System.State] IN ({active_states})`
- Work item details fetched in batch via `GET /_apis/wit/workitems?ids={ids}&$expand=relations`
- Issue-state refresh query uses the same batch work items endpoint filtered to the running issue IDs
- Pagination required for WIQL results; use `$top` parameter and continuation tokens
- Page size default: `200` (ADO WIQL maximum)
- Network timeout: `30000 ms`

Important:

- ADO REST API schema and field names (`System.State`, `System.Title`, etc.) are stable but use
  the versioned API endpoint (`api-version=7.1`) to reduce unintentional drift.

A non-ADO implementation may change transport details, but the normalized outputs must match the
...
- `labels` -> lowercase strings; derived from `System.Tags` (semicolon-separated ADO tag list)
- `blocked_by` -> derived from work item relations where the relation type is
  `System.LinkTypes.BlockedBy`
- `priority` -> integer only, sourced from `Microsoft.VSTS.Common.Priority` (non-integers become null)
- `created_at` and `updated_at` -> parse ISO-8601 timestamps from `System.CreatedDate` and
  `System.ChangedDate`
- `identifier` -> the ADO work item ID (integer), formatted as a string
- `title` -> `System.Title`
- `description` -> `System.Description`
- `state` -> `System.State`
- `url` -> constructed from organization URL, project name, and work item ID, or taken from
  `_links.html.href` when available
...
- `ado_api_request` (transport failures)
- `ado_api_status` (non-2xx HTTP)
- `ado_api_error` (ADO-level error response body)
- `ado_unknown_payload`
- `ado_pagination_error` (continuation token or page integrity error)
...
- If the optional `ado_api` client-side tool extension is implemented, it is still part of
...
- Filtering which Azure DevOps work items, projects, teams, area paths, or other tracker sources
  are eligible for dispatch so untrusted or out-of-scope tasks do not automatically reach the agent.
- Narrowing the optional `ado_api` tool so it can only read or mutate data inside the intended
  project scope, rather than exposing general workspace-wide tracker access.
...
- `tracker.kind` validation enforces currently supported kind (`ado`)
...
- ADO WIQL query filters by project name (`tracker.project_slug`) and active states
- Empty `fetch_issues_by_states([])` returns empty without API call
- Pagination preserves order across multiple pages
- Blockers are normalized from work item relations of type `System.LinkTypes.BlockedBy`
- Labels are normalized to lowercase from `System.Tags`
- Issue state refresh by ID returns minimal normalized issues
- Issue state refresh uses the batch work items endpoint with the running issue IDs
- Error mapping for request errors, non-2xx HTTP, ADO API error responses, malformed payloads

### 17.4 Orchestrator Dispatch, Reconciliation, and Retry

- Dispatch sort order is priority then oldest creation time
- `New` issue with non-terminal blockers is not eligible
- `New` issue with terminal blockers is eligible
...
- If the optional `ado_api` client-side tool extension is implemented:
  - the tool is advertised to the session
  - valid `method`/`path` inputs execute REST API calls against the configured ADO organization
  - valid `wiql` inputs execute WIQL queries against the configured ADO project
  - HTTP 4xx/5xx responses produce `success=false` while preserving the response body
...
- A real tracker smoke test can be run with valid credentials supplied by `ADO_PAT` or a
  documented local bootstrap mechanism (for example `~/.ado_pat`).
...
- Optional `ado_api` client-side tool extension exposes ADO REST API access through the
...
- TODO: Add pluggable issue tracker adapters beyond Azure DevOps.
